...
function executeAction(state, action) {
  ...
  switch (type) {
    ...
    case "apply_financial_plan": output = applyFinancialPlan(state, action); break;
...
// Applies the information gathered by the monthly planner in one audited
// operation. Nothing here creates a real bank transfer or a confirmed income:
// salary/commission remain explicitly planned until the household confirms
// that the transfers happened.
function applyFinancialPlan(state, action) {
  const data = action.data || action;
  const cycle = String(data.cycle || "");
  if (!cycle) throw new Error("Falta el ciclo financiero del plan.");
  const proposedAccounts = Array.isArray(data.accounts) ? data.accounts : [];
  const proposedCards = Array.isArray(data.cards) ? data.cards : [];
  if (!proposedAccounts.length) throw new Error("El plan necesita al menos una cuenta operativa.");

  const key = (value) => String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
  const accountByName = new Map((state.accounts || []).map((account) => [key(account.name), account]));
  const appliedAccounts = proposedAccounts.map((proposal) => {
    const name = String(proposal.name || "").trim();
    if (!name) throw new Error("Una cuenta no tiene nombre.");
    let account = accountByName.get(key(name));
    if (!account) {
      account = { id: makeId(), name, type: String(proposal.type || "bank"), purpose: "", operatingContext: {}, minimumBalance: 0, targetBalance: 0, associatedPayments: [], associatedIncome: [], active: true, createdAt: new Date().toISOString() };
      state.accounts.push(account);
      accountByName.set(key(name), account);
    }
    account.purpose = String(proposal.purpose || account.purpose || "").trim();
    account.operatingContext = proposal.operatingContext || account.operatingContext || {};
    account.minimumBalance = Number(proposal.minimumBalance || 0);
    if (proposal.targetBalance !== undefined) account.targetBalance = Number(proposal.targetBalance || 0);
    account.updatedAt = new Date().toISOString();
    if (Number.isFinite(Number(proposal.balance))) {
      state.accountBalances.push({ id: makeId(), accountId: account.id, balance: Number(proposal.balance), date: String(data.date || new Date().toISOString().slice(0, 10)).slice(0, 10), cycle, reportedBy: action.actor || "shared", createdAt: new Date().toISOString() });
    }
    return account;
  });

  const appliedCards = proposedCards.map((proposal) => {
    const name = String(proposal.name || "").trim();
    if (!name) throw new Error("Una tarjeta no tiene nombre.");
    let card = (state.cards || []).find((item) => key(item.name) === key(name));
    const paymentAccount = proposal.paymentAccountName ? accountByName.get(key(proposal.paymentAccountName)) : null;
    if (!card) {
      card = { id: makeId(), name, purpose: "", paymentAccountId: null, cutoffDay: null, dueDay: null, rate: 0, active: true, installments: [], statements: [], createdAt: new Date().toISOString() };
      state.cards.push(card);
    }
    card.purpose = String(proposal.purpose || card.purpose || "").trim();
    if (paymentAccount) card.paymentAccountId = paymentAccount.id;
    if (proposal.cutoffDay) card.cutoffDay = Number(proposal.cutoffDay);
    if (proposal.dueDay) card.dueDay = Number(proposal.dueDay);
    if (proposal.rate !== undefined) card.rate = Number(proposal.rate || 0);
    const statement = proposal.statement || {};
    if (Object.keys(statement).length) {
      card.statements = [...(card.statements || []).filter((item) => item.month !== String(data.month || "")), {
        id: makeId(), month: String(data.month || new Date().toISOString().slice(0, 7)), totalBalance: Number(statement.totalBalance || 0), statementBalance: Number(statement.statementBalance || 0), minimumPayment: Number(statement.minimumPayment || 0), dueDate: statement.dueDate || "", reportedAt: new Date().toISOString(),
      }];
    }
    card.updatedAt = new Date().toISOString();
    return card;
  ...
  const incomeSpecs = [
    { incomeKind: "salary", amount: Number(data.salaryPlanned || 0) },
    { incomeKind: "commission", amount: Number(data.commissionPlanned || 0) },
  ].filter((item) => item.amount > 0);
  state.incomePlans = Array.isArray(state.incomePlans) ? state.incomePlans : [];
  const incomePlans = incomeSpecs.map((spec) => {
    let plan = state.incomePlans.find((item) => item.cycle === cycle && item.incomeKind === spec.incomeKind);
    if (!plan) { plan = { id: makeId(), cycle, incomeKind: spec.incomeKind, amount: spec.amount, status: "planned", createdAt: new Date().toISOString() }; state.incomePlans.push(plan); }
    else { plan.amount = spec.amount; plan.status = "planned"; plan.updatedAt = new Date().toISOString(); }
    return plan;
  });

  state.monthlyPlans = Array.isArray(state.monthlyPlans) ? state.monthlyPlans : [];
  let plan = state.monthlyPlans.find((item) => item.cycle === cycle);
  const sections = { accounts: appliedAccounts.map((account) => ({ id: account.id, name: account.name, purpose: account.purpose, operatingContext: account.operatingContext || {}, minimumBalance: account.minimumBalance })), cards: appliedCards.map((card) => ({ id: card.id, name: card.name, purpose: card.purpose, dueDay: card.dueDay })), scheduledPayments: (state.scheduledPayments || []).filter((item) => item.active !== false), otherCommitments: String(data.otherCommitments || "").trim() };
  if (!plan) { plan = { id: makeId(), cycle, createdAt: new Date().toISOString() }; state.monthlyPlans.push(plan); }
  Object.assign(plan, { status: "approved", salaryPlanned: Number(data.salaryPlanned || 0), commissionPlanned: Number(data.commissionPlanned || 0), sections, approvedAt: new Date().toISOString(), approvedBy: action.actor || "shared" });
  state.agentMemory.householdProfile = { ...(state.agentMemory.householdProfile || {}), onboarded: true, lastPlanningCycle: cycle, accountsExplained: true, cardsExplained: true, updatedAt: new Date().toISOString() };
  const balances = proposedAccounts.reduce((sum, account) => sum + Number(account.balance || 0), 0);
  return { state, result: { id: plan.id, cycle, accounts: appliedAccounts.length, cards: appliedCards.length, incomePlans: incomePlans.length, plannedIncome: incomePlans.reduce((sum, item) => sum + Number(item.amount || 0), 0), reportedBalances: balances } };
}
