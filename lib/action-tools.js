...
function applyFinancialPlan(state, action) {
  ...
  const appliedAccounts = proposedAccounts.map((proposal) => {
    ...
    if (!account) {
      account = { id: makeId(), name, type: String(proposal.type || "bank"), purpose: "", minimumBalance: 0, targetBalance: 0, associatedPayments: [], associatedIncome: [], active: true, createdAt: new Date().toISOString() };
      state.accounts.push(account);
