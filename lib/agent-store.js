...
async function upsert(table, rows, conflict = "id") {
  ...
  if (!response.ok) {
    ...
    throw new Error(`No pude sincronizar ${table}: ${detail || response.status}.`);
  }
}
...
// Persist the conversational layer separately from the financial state. The
// JSON app state remains the source of truth during migration, while this
// table lets the agent resume a Telegram/web flow without rebuilding it from
// the entire household document.
async function syncConversationState(householdId, state) {
  if (!process.env.SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) return;
  const memory = state.agentMemory || {};
  const tasks = memory.activeTasks || {};
  const sessions = memory.telegramSessions || {};
  const keys = new Set([...Object.keys(tasks), ...Object.keys(sessions)]);
  const now = new Date().toISOString();
  const rows = [...keys].map((key) => {
    const separator = key.indexOf(":");
    const channel = separator > 0 ? key.slice(0, separator) : "telegram";
    const conversationId = separator > 0 ? key.slice(separator + 1) : key;
    const task = tasks[key] || {};
    const messages = Array.isArray(sessions[key]) ? sessions[key].slice(-12) : [];
    return {
      channel,
      conversation_id: conversationId,
      flow_state: task.kind || "idle",
      flow_context: task,
      recent_messages: messages,
      last_specialist_called: task.kind || null,
      turns_in_flow: Number(task.turnsInFlow || messages.length || 0),
      awaiting_user_input: task.step || task.phase || null,
      last_activity_at: task.updatedAt || now,
      updated_at: now,
    };
  });
  await upsert("agent_conversations", rows, "channel,conversation_id");
}

// Email and file imports may exist more than once in the legacy state after
// older retries. The structured schema correctly treats a source ID as one
// financial event, so only mirror the newest representation of that event.
function uniqueMovementRows(rows) {
  const byKey = new Map();
  rows.forEach((row) => {
    const key = row.source_id ? `source:${row.household_id}:${row.source}:${row.source_id}` : `id:${row.id}`;
    const current = byKey.get(key);
    if (!current || String(row.updated_at || row.created_at) >= String(current.updated_at || current.created_at)) byKey.set(key, row);
  });
  return [...byKey.values()];
}

module.exports = { syncStructuredState, syncConversationState, uniqueMovementRows };
