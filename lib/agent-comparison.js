...
function generateComparisonReport(movements, month1, month2) {
  ...
  report += `${month1}: CRC ${fmt(data1.totalExpense)} gastos, CRC ${fmt(data1.totalIncome)} ingresos\n`;
  report += `${month2}: CRC ${fmt(data2.totalExpense)} gastos, CRC ${fmt(data2.totalIncome)} ingresos\n\n`;

  // Trend
  const trend = comparison.expenseChange > 0 ? "📈 ↑" : "📉 ↓";
  report += `${trend} **Cambio total:** ${comparison.expenseChange > 0 ? "+" : ""}CRC ${fmt(comparison.expenseChange)} `;
  ...
  for (const [cat, data] of sortedChanges) {
    ...
    report += `${arrow} ${cat}: ${data.before > 0 ? `CRC ${fmt(data.before)}` : "—"} → CRC ${fmt(data.after)}`;
    if (data.change !== 0) {
...
function generateTrendReport(movements, monthsCount = 3) {
  const trends = detectTrends(movements, monthsCount);
  const months = trends.months;

  let report = `📈 **ANÁLISIS DE TENDENCIAS (últimos ${monthsCount} meses)**\n`;
  report += `${"=".repeat(50)}\n\n`;

  report += `**Tendencia General:** ${trends.overallTrend}\n`;
  report += `**Cambio total:** ${trends.totalChange > 0 ? "+" : ""}CRC ${fmt(trends.totalChange)} `;
  report += `(${trends.totalChangePercent > 0 ? "+" : ""}${trends.totalChangePercent.toFixed(1)}%)\n\n`;

  report += `**Mes a Mes:**\n`;
  for (const month of months) {
    report += `${month.month}: CRC ${fmt(month.totalExpense)} en ${month.transactionCount} transacciones\n`;
  }
  ...
  report += `\n**Análisis:**\n`;
  report += `Gasto promedio: CRC ${fmt(avgSpending)}\n`;
