...
async function analyzePdfWithOpenAI(buffer, filename, categories) {
  ...
  const prompt = [
    ...
    "Formato: {items:[{merchant:string,amount:number,date:string,category:string,note:string,items:[{name:string,quantity:number|null,amount:number|null,category:string}]}]}",
    "Usa fecha YYYY-MM-DD y montos CRC. No inventes datos ilegibles.",
    "Cada artículo debe incluir la categoría más adecuada de esta lista: " + categories.map((item) => item.name).join(", "),
  ].join("\n");
