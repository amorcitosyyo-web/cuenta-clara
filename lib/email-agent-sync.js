...
async function runEmailAgentSync({ existingSourceIds = [], userId = "", email = "", baseUrl }) {
  ...
  if (!mailbox) {
    throw new Error("No pude identificar el correo vinculado para Make. Configura AGENT_OWNER_EMAIL en Vercel o vuelve a vincular la cuenta.");
  }
  // Do not log the mailbox itself: deployment logs must never expose email
  // addresses. These fields are enough to diagnose a Telegram/Make mismatch.
  console.log("Cuenta Clara: iniciando lectura de correo", {
    hasMailbox: true,
    knownSourceIds: Array.isArray(existingSourceIds) ? existingSourceIds.length : 0,
  });
  ...
  console.log("Cuenta Clara: respuesta de Make procesada", {
...
async function readPayload(response) {
  ...
  if (!text) return {};
  // Make emits one bundle at a time. When multiple bundles reach its webhook
  // response they can arrive as consecutive JSON objects instead of one JSON
  // document. normalizeMakeItems already knows how to split that format, but
  // only if it receives the original string. Returning an empty list here was
  // the reason Telegram and cron said “0” while the web button succeeded.
  try { return JSON.parse(text); } catch { return text; }
}

module.exports = { runEmailAgentSync, resolveMailboxEmail, _test: { readPayload } };
