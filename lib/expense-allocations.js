...
function firstCategoryId(categories, candidates) {
  for (const candidate of candidates) {
    const resolved = categoryId(candidate, categories, "");
    if (resolved) return resolved;
  }
  return "";
}

// La IA normalmente identifica bien un alimento, pero una factura de
// supermercado mezcla artículos. Estas señales conocidas evitan que papel,
// higiene o limpieza queden cargados al presupuesto de Alimentación solo por
// venir de PriceSmart, Walmart o un supermercado.
function inferredItemCategory(itemName, categories) {
  const text = normalize(itemName);
  if (!text) return "";
  if (/(papel( toalla| higien)|toalla|servilleta|deterg|cloro|desinfect|suavizante|lavaplat|esponja|bolsa basura|palmolive)/.test(text)) {
    return firstCategoryId(categories, ["limpieza", "hogar"]);
  }
  if (/(colgate|kotex|toalla sanitaria|tampon|shampoo|acondicionador|desodorante|cepillo dental|pasta dental|crema dental|maquill|afeitad|perfume)/.test(text)) {
    return firstCategoryId(categories, ["cuidado personal", "compras personales", "salud"]);
  }
  return "";
}
...
function normalizeReceiptItems(items, categories, fallbackCategory) {
  return (Array.isArray(items) ? items : []).slice(0, 60).map((item) => {
    ...
    return {
      ...
      category: inferredItemCategory(name, categories) || categoryId(item?.category || item?.categoria, categories, fallbackCategory),
    };
...
function expenseAllocations(movement, categories) {
  ...
  // La diferencia normalmente es IVA, descuento o redondeo. Se distribuye de
  // forma proporcional entre los artículos, para que una factura mixta no le
  // cargue todo el IVA a la categoría principal del comercio.
  if (remainder > 0.01) {
    let pending = remainder;
    allocations.forEach((allocation, index) => {
      const share = index === allocations.length - 1
        ? pending
        : Math.round((remainder * allocation.amount / itemTotal) * 100) / 100;
      allocation.amount += share;
      pending = Math.round((pending - share) * 100) / 100;
    });
  }
  return allocations;
}
