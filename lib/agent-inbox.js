...
async function processInboxItems(rawItems, userId) {
  ...
  const changedCategoryIds = [...new Set(result.autoAccepted.map((item) => item.category).filter(Boolean))];
  const budgetAlerts = collectBudgetAlerts(state, { categories, categoryIds: changedCategoryIds });
  await saveAppState(userId, state);
  const report = await notifyTelegram(result, categories);
  if (budgetAlerts.length) await sendTelegram(formatBudgetAlerts(budgetAlerts));
  if (report) {
