...
function generateCategoryBreakdown(movements) {
  ...
  for (const [category, amount] of sorted.slice(0, 8)) {
    ...
    report += `${category}\n${circle} ${percentage}% | CRC ${amount.toFixed(2)}\n\n`;
  }
...
function generateStatistics(movements) {
  ...
  stats += `Gastos totales: CRC ${totalExpense.toFixed(2)}\n`;
  stats += `Ingresos totales: CRC ${totalIncome.toFixed(2)}\n`;
  stats += `Balance: CRC ${(totalIncome - totalExpense).toFixed(2)}\n`;
  stats += `Promedio por transacción: CRC ${average.toFixed(2)}\n`;
  stats += `Gasto máximo: CRC ${max.toFixed(2)}\n`;
  stats += `Gasto mínimo: CRC ${min.toFixed(2)}\n`;
...
function generateComparisonReport(state, month1, month2) {
  ...
  report += `${trend} Gastos:\n`;
  report += `  ${month1}: CRC ${data1.expenses.toFixed(2)}\n`;
  report += `  ${month2}: CRC ${data2.expenses.toFixed(2)}\n`;
