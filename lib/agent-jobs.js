...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (job === "email") {
      ...
      // Email sync persists imports itself. Reload before writing the job log so
      // a cron receipt can never overwrite movements created by Make.
      const refreshed = await getAppState(ownerId);
      recordAutomationRun(refreshed, job, result);
      await saveAppState(ownerId, refreshed);
    } else {
      const state = await getAppState(ownerId);
      if (job === "due") result = await dueJob(state);
      else result = await routineJob(state);
      recordAutomationRun(state, job, result);
...
function recordAutomationRun(state, job, result) {
  state.agentMemory = state.agentMemory || {};
  const runs = Array.isArray(state.agentMemory.automationRuns) ? state.agentMemory.automationRuns : [];
  state.agentMemory.automationRuns = [{
    at: new Date().toISOString(),
    job: String(job || "routine"),
    ok: true,
...
async function dueJob(state) {
  ...
  const pending = (state.scheduledPayments || []).filter((item) => {
    if (item.active === false || (item.paidMonths || []).includes(date.slice(0, 7))) return false;
    return targets.includes(scheduledOccurrence(item, date.slice(0, 7)));
  });
  if (!pending.length) return { notified: 0 };
  await sendTelegram(`📅 Recordatorio de pagos\n\n${pending.map((item) => `• ${item.name}: CRC ${Number(item.amount || 0).toFixed(2)} · vence ${scheduledOccurrence(item, date.slice(0, 7))}`).join("\n")}`);
  return { notified: pending.length };
}

function scheduledOccurrence(payment, month) {
  if (payment.repeat !== "monthly") return String(payment.dueDate || "").slice(0, 10);
...
async function routineJob(state) {
  ...
  const alerts = collectBudgetAlerts(state, { month, categories });
  if (alerts.length) {
    await sendTelegram(formatBudgetAlerts(alerts));
...
module.exports._test = { scheduledOccurrence, recordAutomationRun };
