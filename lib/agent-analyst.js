...
const EXECUTABLE_ACTIONS = new Set([
  "create_movement", "record_income", "set_budget",
...
function executeTool(name, args, state, categories) {
  ...
  if (name === "execute_action") {
    ...
    try {
      // create_movement must keep its fields nested under `data`: the
      // movement's own `type` (expense/income/saving) would otherwise
      // collide with the action's own `type` ("create_movement"). Every
      // other handler reads its fields directly off the action object.
      const payload = args.type === "create_movement"
        ? { type: args.type, data: args.data || {}, actor: "ai-analyst", channel: "telegram" }
