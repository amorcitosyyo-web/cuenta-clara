  ...
  for (const [category, amounts] of Object.entries(categoryStats)) {
    ...
    if (potentialSavings > 10000) {
      suggestions.push({
        ...
        message: `💡 ${category}: Reducir de CRC ${max.toFixed(2)} a CRC ${reductionTarget.toFixed(2)} = Ahorro de CRC ${potentialSavings.toFixed(2)}/mes`
      });
    }
  }
...
function generateGoalReport(state) {
  ...
  report += `💰 **Total potencial de ahorro:** CRC ${totalPotential.toFixed(2)}/mes\n`;
...
function generateMotivation(state) {
  ...
  if (thisMonthExpense < lastMonthExpense) {
    ...
    message += `🎉 ¡Excelente! Este mes gastaste CRC ${savings.toFixed(2)} menos que el anterior (${percent.toFixed(1)}% de reducción).\n`;
    ...
    message += `⚠️ Este mes los gastos aumentaron CRC ${increase.toFixed(2)} (${percent.toFixed(1)}% más que el mes pasado).\n`;
