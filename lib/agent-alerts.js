...
function detectAnomalies(movements, category, newAmount) {
  ...
  return {
    ...
    recommendation: isAnomaly
      ? `⚠️ Este gasto de CRC ${newAmount.toFixed(2)} es ${Math.abs(percentageAboveAvg).toFixed(0)}% más alto que tu promedio de CRC ${stats.avg.toFixed(2)} en ${category}.`
      : null
  };
}
  ...
  for (const [category, limit] of Object.entries(budgets)) {
    ...
    if (percentage >= 100) {
      alerts.push({
        ...
        message: `🚨 ${category}: Excediste el presupuesto. Gastaste CRC ${spent.toFixed(2)} de CRC ${limit.toFixed(2)}`
      });
      ...
      alerts.push({
        ...
        limit: limit.toFixed(2),
        percentage: percentage.toFixed(0),
        message: `⚠️ ${category}: Vas al ${percentage.toFixed(0)}% del presupuesto (CRC ${spent.toFixed(2)} de CRC ${limit.toFixed(2)})`
      });
    }
  }
...
function suggestLimits(state) {
  ...
  for (const category of categories) {
    ...
    suggestions[category] = {
      ...
      maxSpent: max.toFixed(2),
      suggestedLimit: suggestedLimit.toFixed(2),
      message: `${category}: Basado en 3 meses, sugerimos límite de CRC ${suggestedLimit.toFixed(2)}`
    };
  }
