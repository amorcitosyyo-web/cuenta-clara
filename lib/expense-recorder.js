...
function parseSpanishAmount(text) {
  ...
  const units = { cero: 0, un: 1, uno: 1, una: 1, dos: 2, tres: 3, cuatro: 4, cinco: 5, seis: 6, siete: 7, ocho: 8, nueve: 9, diez: 10 };
  const match = normalized.match(/\b(cero|un|uno|una|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)\s+mil\b/);
  if (match) return units[match[1]] * 1000;
  if (/\bmil\b/.test(normalized)) return 1000;
  return null;
}
...
function extractMerchant(text, amountToken) {
  ...
  const explicit = source.match(/\b(?:en|a|para)\s+(?:el|la|los|las|un|una)?\s*([^,.;!?]+?)(?=\s+(?:de|del|por|para|categoria|categoría|fecha|hoy|ayer)\b|[,.;!?]|$)/i);
...
function parseExpenseRequest(text, { categories = [], today = new Date(), requireVerb = false } = {}) {
  ...
  const amount = amountMatch ? parseAmount(amountMatch[0]) : parseSpanishAmount(source);
...
function isExpenseRequest(text) {
  return hasExpenseVerb(String(text || "")) && (/(?:₡|crc\s*)?\d[\d.,]*/i.test(String(text || "")) || parseSpanishAmount(text) !== null);
}

module.exports = { parseExpenseRequest, isExpenseRequest, _test: { parseAmount, parseSpanishAmount, dateFromText, extractMerchant, categoryFor } };
