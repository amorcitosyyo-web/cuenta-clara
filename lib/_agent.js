...
async function saveAppState(userId, data) {
  ...
  // Conversation memory is additive and independently recoverable. A missing
  // table must not block saving the financial state during rollout.
  try {
    const { syncConversationState } = require("./agent-store");
    await syncConversationState(userId, normalized);
  } catch (error) {
    console.error("Conversation memory sync error:", error);
  }
...
async function sendTelegram(message, keyboard = []) {
  ...
  const formatted = String(message || "").replace(/\*\*(.+?)\*\*/g, "*$1*");
  const send = (text, parseMode) => fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
    method: "POST",
  ...
  const result = await send(formatted, "Markdown");
  if (result?.ok === false) return send(message, null);
  return result;
}
