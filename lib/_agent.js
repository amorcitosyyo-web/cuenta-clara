...
// Supabase can occasionally return a 502/503/504 even when the data itself is
// healthy. Telegram webhooks must not turn that short outage into a dead-end
// for the household, so state reads and writes make a few fast safe retries.
// The cache is only a warm-instance fallback: it is never written back unless
// Supabase has first accepted the latest state.
...
function isTransientSupabaseFailure(response) {
  return !response || TRANSIENT_SUPABASE_STATUSES.has(response.status);
}

async function fetchSupabaseState(url, options) {
  let lastError;
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      const response = await fetch(url, options);
      if (response.ok || !isTransientSupabaseFailure(response) || attempt === 2) return response;
      lastError = new Error(`Supabase respondió ${response.status}`);
    } catch (error) {
      lastError = error;
  ...
  throw lastError || new Error("No pude comunicarme con Supabase");
}
...
function normalizeState(data) {
  ...
  return {
    ...
    agentMemory: {
      ...
      pendingActions: {},
      movementDrafts: {},
      pausedMovementDrafts: {},
      recurringPaymentDrafts: {},
      recurringCandidates: {},
      telegramReports: [],
      chatHistory: [],
      telegramSessions: {},
      ...
      movementDrafts:
        state.agentMemory?.movementDrafts && typeof state.agentMemory.movementDrafts === "object" && !Array.isArray(state.agentMemory.movementDrafts)
          ...
          : {},
      pausedMovementDrafts:
        state.agentMemory?.pausedMovementDrafts && typeof state.agentMemory.pausedMovementDrafts === "object" && !Array.isArray(state.agentMemory.pausedMovementDrafts)
          ? state.agentMemory.pausedMovementDrafts
          : {},
      recurringPaymentDrafts:
        state.agentMemory?.recurringPaymentDrafts && typeof state.agentMemory.recurringPaymentDrafts === "object" && !Array.isArray(state.agentMemory.recurringPaymentDrafts)
          ? state.agentMemory.recurringPaymentDrafts
          : {},
      recurringCandidates:
        state.agentMemory?.recurringCandidates && typeof state.agentMemory.recurringCandidates === "object" && !Array.isArray(state.agentMemory.recurringCandidates)
          ? state.agentMemory.recurringCandidates
          : {},
      ...
      automationSettings:
        state.agentMemory?.automationSettings && typeof state.agentMemory.automationSettings === "object" && !Array.isArray(state.agentMemory.automationSettings)
          ...
          : {},
      automationRuns: Array.isArray(state.agentMemory?.automationRuns) ? state.agentMemory.automationRuns.slice(-100) : [],
      undoActions:
      ...
      pausedActions:
        state.agentMemory?.pausedActions && typeof state.agentMemory.pausedActions === "object" && !Array.isArray(state.agentMemory.pausedActions)
          ...
          : {},
      budgetAlertLevels:
        state.agentMemory?.budgetAlertLevels && typeof state.agentMemory.budgetAlertLevels === "object" && !Array.isArray(state.agentMemory.budgetAlertLevels)
          ? state.agentMemory.budgetAlertLevels
          : {},
    },
...
async function getAppState(userId) {
  try {
    const response = await fetchSupabaseState(`${process.env.SUPABASE_URL}/rest/v1/app_states?user_id=eq.${encodeURIComponent(userId)}&select=data`, {
      headers: {
    ...
    appStateCache.set(userId, state);
    return state;
  } catch (error) {
    const cached = appStateCache.get(userId);
    if (cached) {
      console.error("Supabase state read failed; using a warm verified cache:", error);
...
async function saveAppState(userId, data) {
  ...
  const response = await fetchSupabaseState(`${process.env.SUPABASE_URL}/rest/v1/app_states?on_conflict=user_id`, {
    method: "POST",
    headers: {
      apikey: process.env.SUPABASE_SERVICE_ROLE_KEY,
      Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`,
    ...
    body: JSON.stringify({ user_id: userId, data: normalized }),
  });
  if (!response.ok) throw new Error("No pude guardar los datos del agente");
  appStateCache.set(userId, normalized);
  if (data?.meta?.structuredStorageEnabled) {
    const { syncStructuredState } = require("./agent-store");
    // The legacy state is the active source during the migration. A mirror
    // error must never make Telegram lose a valid reply or a confirmed change.
    // It remains visible in Vercel logs and will retry on the next save.
    try {
      await syncStructuredState(userId, normalizeState(data));
    } catch (error) {
      console.error("Structured mirror sync error:", error);
    }
  }
}
...
// MULTI-AGENT CONVERSATIONAL STATE HELPERS
// These functions manage persistent conversation flow across turns.

async function loadConversation(channel, conversationId) {
  try {
    const url = `${process.env.SUPABASE_URL}/rest/v1/agent_conversations?channel=eq.${encodeURIComponent(channel)}&conversation_id=eq.${encodeURIComponent(String(conversationId))}`;
    const response = await fetchSupabaseState(url, {
      headers: {
        apikey: process.env.SUPABASE_SERVICE_ROLE_KEY,
        Authorization: `Bearer ${process.env.SUPABASE_SERVICE_ROLE_KEY}`,
      },
    ...
    const rows = await response.json().catch(() => []);
    if (!response.ok) throw new Error("No pude cargar el estado conversacional");
    return rows?.[0] || null;
  } catch (error) {
...
async function startConversation(channel, conversationId, flowState, initialContext) {
  try {
    const url = `${process.env.SUPABASE_URL}/rest/v1/agent_conversations?on_conflict=channel,conversation_id`;
    const response = await fetchSupabaseState(url, {
      method: "POST",
      headers: {
        apikey: process.env.SUPABASE_SERVICE_ROLE_KEY,
      ...
      body: JSON.stringify({
        ...
        flow_context: initialContext,
        turns_in_flow: 0,
        started_at: new Date().toISOString(),
        last_turn_at: new Date().toISOString(),
      }),
    });
    if (!response.ok) throw new Error("No pude iniciar la conversación");
    return true;
  } catch (error) {
...
async function updateConversation(channel, conversationId, updates) {
  try {
    const url = `${process.env.SUPABASE_URL}/rest/v1/agent_conversations?channel=eq.${encodeURIComponent(channel)}&conversation_id=eq.${encodeURIComponent(String(conversationId))}`;
    const response = await fetchSupabaseState(url, {
      method: "PATCH",
    ...
    if (!response.ok) throw new Error("No pude actualizar la conversación");
    return true;
  } catch (error) {
...
async function endConversation(channel, conversationId) {
  return updateConversation(channel, conversationId, {
    flow_state: "idle",
    flow_context: {},
...
function addToRecentMessages(current = [], newMessage, maxLength = 10) {
  const updated = [...current, {
    role: newMessage.role,
    text: String(newMessage.text || "").slice(0, 500),
...
function detectIntent(text) {
  ...
  if (/\b(plan|organiz|mes|ciclo|quincena|presupuest)\b/.test(normalized)) return "plan";
  if (/\b(analiz|gasto|categoria|cuanto|cuánto|gastamos|gastaré)\b/.test(normalized)) return "analyze_expenses";
  if (/\b(ingreso|salario|comisi[oó]n|entrada|recib[íi]|ganancia|pago)\b/.test(normalized)) return "income";
  if (/\b(men[uú]|ayuda|opciones|comando|qué puedes|puedes hacer)\b/.test(normalized)) return "menu";
  if (/\b(estado|resumen|total|cuanto|cuentas|saldo)\b/.test(normalized)) return "status";
  if (/\b(cuenta|banco|tarjeta|dinero)\b/.test(normalized)) return "accounts";
...
module.exports = {
  ...
  saveAppState,
  sendTelegram,
  // Multi-agent conversation functions
  loadConversation,
  startConversation,
  updateConversation,
  endConversation,
