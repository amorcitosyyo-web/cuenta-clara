...
function planningQuestionReply(task, answer) {
  ...
  if (!/(\?|\b(que|que es|por que|porque|como|explica|ocupas|necesitas|para que)\b)/.test(text)) return null;
...
module.exports = { CONFIRMATION_REQUIRED, costaRicaDate, cycleFor, executeProposedAction, findDuplicate, getTask, householdSnapshot, menu, planningPrompt, runAgentTurn, setTask, taskKey, undoAction, _test: { buildAgentMessages, parseModelResponse, continuePlanning, deleteMovementRequest, accountOperatingContext } };
