...
async function runAgentTurn({ state, channel, conversationId, actor = "shared", text, intent = null, history = [], quotedContext = "" }) {
  ...
  if (/\b(reporte|gráfico|análisis|estadística|visuales?)\b/i.test(normalized)) {
    return { text: generateFullReport(state) };
  }

  if (/\b(tendencia|trend|crecimiento|comparar|comparativa)\b/i.test(normalized)) {
    return { text: generateTrendReport(state.movements || []) };
  }

  if (/\b(meta|metas|ahorro|objetivo|goal|ahorr|inteligente)\b/i.test(normalized)) {
    return { text: generateGoalReport(state) };
  }

  if (/\b(motivación|motiv|ánimo|progreso|celebr|felicit)\b/i.test(normalized)) {
