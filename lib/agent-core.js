...
function getSpecialist(flowState) {
  const specialists = {
    ...
    "analyzing_expenses": require("./agent-expense-analyzer-v2"),
    "classifying_expense": require("./agent-classifier-v2"),
    "recording_income": require("./agent-income-recorder"),
    "confirming_action": require("./agent-action-executor"),
  };
...
async function runAgentTurn({
  ...
  if (flowState !== "idle") {
    ...
    if (specialist) {
      try {
        ...
        // Execute action if specialist returns one
        if (response?.action) {
          try {
            const { result } = executeAction(state, { ...response.action, actor, channel, idempotencyKey: makeId() });
            response.text = response.text || `✅ Acción ejecutada: ${describeResult(response.action.type, result)}`;
          } catch (error) {
  ...
  if (flowState === "idle") {
    ...
    if (intent === "income") {
      await startConversation(channel, conversationId, "recording_income", {
        stage: "asking_amount",
      });
      return {
        text: "💵 Vamos a registrar un ingreso. ¿Cuál es el monto?",
        nextQuestion: "Monto"
      };
    }
...
function describeResult(type, result) {
  if (type === "create_movement" || type === "record_income") return `${result.merchant || "Movimiento"} por CRC ${Number(result.amount || 0).toFixed(2)}`;
  if (type === "update_movement") return `${result.merchant || "Movimiento"} actualizado`;
  if (type === "create_account") return `Cuenta "${result.name}" creada`;
  if (type === "create_card") return `Tarjeta "${result.name}" creada`;
