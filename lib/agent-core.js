...
async function runAgentTurn({ state, channel, conversationId, actor = "shared", text, intent = null, history = [], quotedContext = "" }) {
  ...
  const keywordAgentHint = /\b(reporte|gráfico|análisis|estadística|visuales?|tendencia|trend|crecimiento|comparar|comparativa|meta|metas|ahorro|objetivo|goal|ahorr|inteligente|motivación|motiv|ánimo|progreso|celebr|felicit)\b/i.test(normalized);
  if (keywordAgentHint) {
    const routed = await classifyAgentIntent(question).catch(() => null);
    const resolvedAgent = routed && routed.confidence >= 0.5 ? routed.intent : null;

    if (resolvedAgent === "report" || /\b(reporte|gráfico|análisis|estadística|visuales?)\b/i.test(normalized)) {
      return { text: generateFullReport(state) };
    }
    if (resolvedAgent === "trend" || /\b(tendencia|trend|crecimiento|comparar|comparativa)\b/i.test(normalized)) {
      return { text: generateTrendReport(state.movements || []) };
    }
    if (resolvedAgent === "goals" || /\b(meta|metas|ahorro|objetivo|goal|ahorr|inteligente)\b/i.test(normalized)) {
      return { text: generateGoalReport(state) };
    }
    if (resolvedAgent === "motivation" || /\b(motivación|motiv|ánimo|progreso|celebr|felicit)\b/i.test(normalized)) {
