...
const { detectAnomalies, checkBudgetExceeded, formatAlertMessage } = require("./agent-alerts");
const { generateFullReport, generateComparisonReport } = require("./agent-reports");
const { generateTrendReport } = require("./agent-comparison");
const { generateGoalReport, generateMotivation } = require("./agent-goals");
...
function isNewTopic(text, task) {
  ...
  if (/^(hola|holaa|holaaa|buenas|buenos dias|buenas tardes|buenas noches|hey|que tal|qué tal)$/.test(normalized)) return true;
  const hints = { movement: /\b(reporte|correo|planificar|cuenta|tarjeta|meta|presupuesto)\b/, income: /\b(reporte|correo|planificar|gasto|cuenta|tarjeta)\b/, planning: /\b(reporte|correo|gasto|factura|meta|metas|ahorro|motivación|motiv|tendencia|comparar|progreso|animo|ánimo)\b/ };
...
function normalizeAccountAction(state, action) {
  if (String(action?.type || "") !== "update_account") return action;
  const data = action.data || action;
  const key = (value) => normalizeText(value);
  const account = action.id
    ? (state.accounts || []).find((item) => item.id === action.id)
    : (state.accounts || []).find((item) => key(item.name) === key(data.name));
  if (account) return { ...action, id: account.id };
  // Some model responses interpret “create these accounts” as an update
  // because the account list is currently empty. Convert that proposal into
  // a valid creation before it reaches confirmation or execution.
  return { ...action, type: "create_account", data: { ...data, name: data.name || data.accountName } };
}

function executeProposedAction({ state, channel, conversationId, actor, action }) {
  action = normalizeAccountAction(state, action);
  ...
  let responseText = `✅ Listo. ${describeResult(type, result)}`;
  const alertMessage = checkAlertsForResult(state, type, result);
  if (alertMessage) {
    responseText += `\n\n${alertMessage}`;
  }

  return { text: responseText, buttons: undoId ? [[{ text: "↩️ Deshacer", callback_data: `agent:undo:${undoId}` }, { text: "✏️ Editar", callback_data: `agent:edit:${result.id}` }]] : [] };
}
...
function checkAlertsForResult(state, type, result) {
  if (type !== "create_movement" && type !== "record_income") return null;

  const alerts = [];
  const category = result.category || "Otro";
  const amount = Number(result.amount || 0);

  // Check for anomalies
  const anomaly = detectAnomalies(state.movements || [], category, amount);
  if (anomaly?.isAnomaly) {
    alerts.push(anomaly.recommendation);
  }

  // Check budget overages
  const budgetAlerts = checkBudgetExceeded(state);
  if (budgetAlerts.length > 0) {
    alerts.push(formatAlertMessage(budgetAlerts));
  }
...
async function classifyAgentIntent(question) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) return null;
  const promptLines = [
    'Eres el router de un bot financiero personal de Costa Rica con varios agentes especializados.',
    'Decide cual agente debe atender el mensaje del usuario.',
    'Agentes:',
    '- report: pide ver reportes, graficos, desglose visual o estadisticas generales de sus finanzas',
    '- trend: pide ver tendencias, comparar meses, saber si gasta mas o menos con el tiempo',
    '- goals: pide metas de ahorro, oportunidades de ahorro o sugerencias para ahorrar dinero',
    '- motivation: pregunta como va su progreso, pide animo, felicitacion o balance de su comportamiento financiero',
    '- other: cualquier otra cosa: registrar movimientos, preguntas, conversacion, planificacion, cuentas, etc',
    'Responde SOLO JSON: {"intent":"report|trend|goals|motivation|other","confidence":0-1}',
...
async function runAgentTurn({ state, channel, conversationId, actor = "shared", text, intent = null, history = [], quotedContext = "" }) {
  ...
  const skipAiRouter = /(agrega|agregar|agregalo|agrégalo|registra|registrar|anota|anotar|añade|anadir|añadir|mete|meter|cancelar|cancela)/.test(normalized);
  const routed = !skipAiRouter ? await classifyAgentIntent(question).catch(() => null) : null;
  {
    const resolvedAgent = routed && routed.confidence >= 0.5 ? routed.intent : null;

    if (resolvedAgent === "report" || (!routed && /\b(reporte|gráfico|análisis|estadística|visuales?)\b/i.test(normalized))) {
      return { text: generateFullReport(state) };
    }
    if (resolvedAgent === "trend" || (!routed && /\b(tendencia|trend|crecimiento|comparar|comparativa)\b/i.test(normalized))) {
      const requestedMonths = Number(normalized.match(/\b(\d{1,2})\s*mes(?:es)?\b/)?.[1]) || 3;
      return { text: generateTrendReport(state.movements || [], Math.min(Math.max(requestedMonths, 1), 24)) };
    }
    if (resolvedAgent === "goals" || (!routed && /\b(meta|metas|ahorro|objetivo|goal|ahorr|inteligente)\b/i.test(normalized))) {
      return { text: generateGoalReport(state) };
    }
    if (resolvedAgent === "motivation" || (!routed && /\b(motivación|motiv|ánimo|progreso|celebr|felicit)\b/i.test(normalized))) {
      return { text: generateMotivation(state) };
    }
  }
  ...
  if (task?.kind === "planning" && task.phase !== "review" && /^(cancelar|cancela|salir|detener|olvidalo|olvídalo|ya no|deja eso)$/i.test(normalized)) {
    setTask(state, channel, conversationId, null);
    return { text: "❌ Cancelé la planificación. No guardé nada." };
  }
  if (task && isNewTopic(question, task)) return { text: "Hay una tarea pendiente. ¿Quieres continuarla, pausarla para atender esta nueva solicitud, o cancelarla?", buttons: [[{ text: "▶️ Continuar", callback_data: "agent:task:continue" }, { text: "⏸️ Pausar y cambiar", callback_data: "agent:task:pause" }, { text: "❌ Cancelar", callback_data: "agent:task:cancel" }]] };
  ...
  const analystReply = await runAnalystAgent(question, state, getCategories(state), costaRicaDate(), history).catch(() => null);
  if (analystReply) return { text: analystReply };
  const snapshot = householdSnapshot(state);
  try {
