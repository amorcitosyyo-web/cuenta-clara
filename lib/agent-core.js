...
function householdSnapshot(state) {
  ...
  const spendingByCategory = Object.entries(expenses.reduce((totals, item) => {
...
function planningPrompt(state, channel, conversationId) {
  ...
  return { text: `🗓️ Organicemos el ciclo ${cycle.start} al ${cycle.end}. No registraré dinero ni transferencias hasta que aprueben el resumen final.\n\nPaso 1 de 6 — ¿Cuántas cuentas operativas usan? Incluye cuentas donde reciben, pagan o ahorran; no incluyas tarjetas. Responde solo con el número.` };
}
...
function continuePlanning(state, channel, conversationId, text, actor) {
  ...
  if (task.phase === "account_count") { const count = planCount(answer); if (!count) return { text: "Necesito el número de cuentas operativas; por ejemplo: 2." }; task.accountCount = count; task.accountIndex = 0; task.phase = "account_details"; setTask(state, channel, conversationId, task); return { text: `Cuenta 1 de ${count}: escribe nombre y propósito. Ejemplo: “BAC principal — pagos del hogar”.` }; }
  if (task.phase === "account_details") { const account = planName(answer); if (!account) return { text: "Escribe el nombre y propósito. Ejemplo: “BAC principal — pagos del hogar”." }; task.currentAccount = account; task.phase = "account_balance"; setTask(state, channel, conversationId, task); return { text: `¿Cuál es el saldo actual de ${account.name}? Responde solo el monto en colones.` }; }
  if (task.phase === "account_balance") { const balance = planMoney(answer); if (balance === null) return { text: "Dime el saldo como monto, por ejemplo ₡125000." }; task.currentAccount.balance = balance; task.phase = "account_minimum"; setTask(state, channel, conversationId, task); return { text: `¿Cuál es el saldo mínimo que quieren dejar en ${task.currentAccount.name}? Escribe 0 si no aplica.` }; }
  if (task.phase === "account_minimum") { const minimumBalance = planMoney(answer); if (minimumBalance === null || minimumBalance < 0) return { text: "Dime un saldo mínimo válido, o 0." }; task.accounts.push({ ...task.currentAccount, minimumBalance }); delete task.currentAccount; task.accountIndex += 1; if (task.accountIndex < task.accountCount) { task.phase = "account_details"; setTask(state, channel, conversationId, task); return { text: `Cuenta ${task.accountIndex + 1} de ${task.accountCount}: nombre y propósito.` }; } task.phase = "card_count"; setTask(state, channel, conversationId, task); return { text: "Paso 2 de 6 — ¿Cuántas tarjetas de crédito usan? Responde 0 si no tienen." }; }
...
async function runAgentTurn({ state, channel, conversationId, actor = "shared", text, intent = null, history = [], quotedContext = "" }) {
  ...
  const planningReply = continuePlanning(state, channel, conversationId, question, actor);
...
module.exports = { CONFIRMATION_REQUIRED, costaRicaDate, cycleFor, executeProposedAction, findDuplicate, getTask, householdSnapshot, menu, planningPrompt, runAgentTurn, setTask, taskKey, undoAction, _test: { buildAgentMessages, parseModelResponse, continuePlanning } };
