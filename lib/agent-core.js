...
async function askModel(text, state, snapshot, history = [], quotedContext = "") {
  ...
  const response = await fetch("https://api.openai.com/v1/chat/completions", { method: "POST", headers: { Authorization: `Bearer ${process.env.OPENAI_API_KEY}`, "Content-Type": "application/json" }, body: JSON.stringify({ model, store: false, temperature: 0.2, max_tokens: 380, messages: buildAgentMessages(text, snapshot, history, quotedContext), tools: toolDefinitions, tool_choice: "auto" }) });
...
module.exports = { CONFIRMATION_REQUIRED, costaRicaDate, cycleFor, executeProposedAction, findDuplicate, getTask, householdSnapshot, menu, planningPrompt, runAgentTurn, setTask, taskKey, undoAction, _test: { buildAgentMessages } };
