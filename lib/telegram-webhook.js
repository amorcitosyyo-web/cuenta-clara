...
async function analyzeTelegramImage(message, caption, imageFileIds = null) {
  ...
  try {
    ...
    if (!parsed?.isReceipt) {
      ...
      if (recovery.ok && recoveryAnswer) parsed = parseReceiptVision(recoveryAnswer) || parsed;
    }
...
module.exports._test = { findRecurringPaymentRequest, isIndependentFinancialRequest, detectRecurringCandidates, isRecurringAnalysisRequest, detectEmailIntent, findCategoryDetailRequest, summarizeCategoryMovements, summarizeCategoryMaximum, findClassificationReview, isMonthMovementsQuestion, findOperationalAction, handleMovementDraft, findDeleteRequest, findReceiptDuplicate, findPossibleReceiptDuplicate, canonicalMerchant, merchantSimilarity };
