...
function findReceiptDuplicate(data, state) {
  ...
  const candidates = (state.movements || []).filter((item) => {
    ...
    return item.type === "expense" && item.date === data.date &&
      ...
      (existingMerchant === merchant || existingMerchant.includes(merchant) || merchant.includes(existingMerchant));
  });
...
module.exports._test = { findRecurringPaymentRequest, isIndependentFinancialRequest, detectRecurringCandidates, isRecurringAnalysisRequest, detectEmailIntent, findCategoryDetailRequest, summarizeCategoryMovements, summarizeCategoryMaximum, findClassificationReview, isMonthMovementsQuestion, findOperationalAction, handleMovementDraft, findDeleteRequest, findReceiptDuplicate, canonicalMerchant };
