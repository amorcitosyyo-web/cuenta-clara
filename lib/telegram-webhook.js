...
function findRecurringPaymentRequest(question, state, sessionKey) {
  ...
  if (!dueDay || dueDay > 31) {
    return {
      status: "incomplete",
      message: `Encontré ${related.length} pago${related.length === 1 ? "" : "s"} de Claro/internet en ${latestMonth || "los registros"}: ${related.map((item) => `CRC ${Number(item.amount || 0).toFixed(2)} el ${item.date}`).join(" y ")}.\n\nPropuesta: “${name}” por CRC ${amount.toFixed(2)} cada mes. ¿Qué día del mes se paga?`,
    };
  }
  ...
  return {
    ...
    preview: `Crearé el pago recurrente “${name}” por CRC ${amount.toFixed(2)}, el día ${dueDay} de cada mes, en ${category?.name || "Teléfono e internet"}.`,
  };
}
...
module.exports._test = { findRecurringPaymentRequest, isIndependentFinancialRequest };
