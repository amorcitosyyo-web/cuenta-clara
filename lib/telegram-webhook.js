...
function isMonthMovementsQuestion(question) {
  ...
  if (/\b(riesgo|principal|analiza|analisis|estado financiero|salud financiera|recomienda|recomendacion|consejo|prioridad)\b/.test(text)) return false;
...
function summarizeCategoryMaximum(state, category, period) {
  ...
  return `El gasto más alto de ${category.name} en el ${period.label} es ${movement.merchant || "Sin comercio"}: ${money(movement.amount)} el ${movement.date}.`;
}
...
module.exports._test = { findRecurringPaymentRequest, isIndependentFinancialRequest, detectRecurringCandidates, isRecurringAnalysisRequest, detectEmailIntent, findCategoryDetailRequest, summarizeCategoryMovements, summarizeCategoryMaximum, findClassificationReview };
