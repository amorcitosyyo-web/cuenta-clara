...
async function replyAsAgent(message, text, request = null) {
  ...
  const emailIntent = detectEmailIntent(question, state, sessionKey);
...
function detectEmailIntent(question, state, sessionKey = "") {
  ...
  const contextualRetry = /(de nuevo|otra vez|nuevamente|reintenta|intentalo|inténtalo)/.test(normalized) &&
    /(correo|email|make|bandeja|movimientos nuevos)/.test(recentConversation);
