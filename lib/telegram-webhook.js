...
async function analyzeTelegramImage(message, caption, imageFileIds = null) {
  ...
  try {
    ...
    for (const fileId of fileIds) {
      ...
      // Recibos muy altos tienen letra pequeña. `high` evita que el proveedor
      // reduzca la imagen antes de intentar leer sus líneas y totales.
      imageParts.push({ type: "image_url", image_url: { url: `data:${mime};base64,${bytes}`, detail: "high" } });
    }
    const state = await getState();
    ...
    const prompt = [
      ...
      fileIds.length > 1 ? "Las imágenes son páginas del mismo comprobante. Únelas: usa todos los artículos y un único total final." : "",
